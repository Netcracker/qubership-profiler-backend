...
(function() {
    ...
    function invokeLater(runnable, timeout) {
        var caller = invokeLater.caller;
    ...
    function getScrollDimensions() {
        ...
        var scrollDim = { width: div.width() - div[0].clientWidth, height: div.height() - div[0].clientHeight };
        div.remove();
        return scrollDim;
    }

    var escapeHTML = ESCUtils.escapeHTML;

    var escapeRegExp = ESCUtils.escapeRegExp;
    ...
    if (app.name == 'CallList')
        (function() {
            ...
            function filterOutProxyRequests(item, tags, folder) {
                ...
                str = tags.t[item[C_METHOD]][0].toLowerCase();
                var urlString = "";
                if(params && tags && tags.r && tags.r['web.url']){
                    urlString = params[tags.r['web.url']];
                }

                for (i = 0; i < defaultHiddenRows.length; i++){
            ...
            function init() {
                ...
                if (!getState('timelineBottom')) // default value on first open
            ...
            function updateTimeline() {
                ...
                if (!timeline) {
                    ...
                    links.events.addListener(timeline, 'rangechanged', function () {
                        invokeLater(Timeline__rangeChanged, 300);
                    });
                    ...
                    $timeline.resizable({
                        ...
                        , resize: function (event, ui) {
                            invokeLater(resizeCallList, 300);
            ...
            function initGrid() {
                ...
                grid = new Slick.Grid($grid, dataView.rows, gridColumns, gridOptions);
                ...
                grid.onColumnsReordered = function() {
                    var $title = $('#' + gridId + 'title');
                    $('<label/>').attr({'for' : 'qry', title: 'Use \'/\' or \'s\' shortcut to start search'})
                        .append($('<span/>').text(', '))
                        .append($('<span/>').attr({style: 'text-decoration: underline'}).text('s'))
                        .append($('<span/>').text('earch: '))
                        .add(
                            $('<input type="search"/>').attr({
                                    id: 'qry'
                                    , name: 'qry'
                                    , value: filter_string
                                    , placeholder: 'use + for mandatory, - to exclude, or "exact phrase" filtering'
                                    , title: 'Search tips: <<+clust1 sysadm administrator>> lists all (sysadm OR administrator requests) made to clust1 node.\n'
                                ...
                                .keyup(FilterString_onKeyUp).blur(FilterString_onBlur).click(function () {
                                FilterString_onKeyUp.call(this);
                                invokeLaterCancel(FilterString_onKeyUp);
                            })
                            .bind("selectstart.ui", function(event) {
                                // The search field is placed in a slickgrid header that prevents selection (so headers can be dragged)
                                // Here we enable selection events for the search field, and stop propagation so slickgrid does not
                                // know selection was attempted
                ...
                grid.onColumnsResized = function() {
                    ...
                    var newWidth = Math.max($title.width() - $qry.position().left, 20);

                    $qry.width(newWidth);
                };

                grid.onColumnsReordered();
...
//                 var tags = folderContents[dataContext[C_FOLDER_ID]].tags;
            ...
            function Configuration__configFile_reloadStatus(params) {
                ...
                invokeLater(Configuration__configFile_reload, 2000);
            }
            ...
            function ThreadDumps__scheduleUpdateFileSize() {
                ...
                if ($(this).val() == '')
                    ...
                    invokeLater(ThreadDumps__updateFileSize, 1000);
            }
    ...
    if (app.name == 'CallTree')
        (function() {
            ...
            CT.defaultCategories = [
                '# Weblogic WebService over JMS',
                'weblogic.wsse weblogic.wsee.server.jms.JmsQueueListener.onMessage',
                '',
                '# WebService implementation',
                'solutions.webservice >weblogic.wsee.component.pojo.JavaClassComponent.invoke',
                '',
                ...
                '',
                '# Custom servlet code',
                'solutions.servlet.unsorted >weblogic.servlet.internal.WebAppServletContext$ServletInvocationAction.run',
                '',
                '# Custom JSP code',
                ...
                '',
                '# AutoInstaller',
                'restart.start_server .start_server()',
                'restart.stop_server .stop_server()',
                'execute_sql.sqlplus .oracle_execute_sql()',
                'execute_sql.sqlldr .execute_sqlldr()',
                'execute_sql.validate_sql_logs .validate_sql_logs()',
                'execute_sql.other build.execute_sql()',
                'offlineImport .offlineImport()',
                'offlineImport .postponed_offlineImport()',
                'ai.validateWeblogicConnection validateWeblogicConnection',
                'ai.check_wl_server .check_wl_server()',
                'ai.configureProject org.apache.tools.ant.ProjectHelper.configureProject',
                'ai.configureProject ai.configureProject org.apache.tools.ant.taskdefs.Ant.initializeProject',
                'ai.toolsinstaller .with-ti_start()',
                'ai.portScanner com.netcracker.tools.internal.installer.PortScanner.execute',
                'ai.get_constraints_psupai1633 build_toms.generate_constraints_details()',
                'collect_results AutoInstaller.build.endmost()',
                'post_start_steps org.apache.tools.ant.Target_.stepJSP()',
                'xml_tasks build_xml.xml_'
            ].join('\n');
            ...
            var javaModules = [
                ...
                ['core.ejb.framework', 'com.netcracker.ejb.framework.'],
                ...
                if (value !== null) {
                    ...
                    if (tagName == 'sql' || tagName == 'sql.monitor' || tagName == 'binds' || tagName == 'mdx' || tagName == 'cassandra.query') {
                        ...
                        for (var key_idx = 0; key_idx < keys.length; key_idx++) {
                            ...
                            var values = qs.values(keys[key_idx]);
            ...
            function init() {
                ...
                $(document).mouseover(function(event) {
                    ...
                    $target.parent().andSelf().filter('.ui-button:not(.ui-state-disabled)').addClass('ui-state-hover');
                    var cc = ctrl_pressed;
                    ctrl_pressed = event.ctrlKey || event.metaKey;
                    if (ctrl_pressed) {
                        IDE_highlight(event);
                    } else if (cc) $('#ide').hide();
                    var $parents = $target.firstParents(6).andSelf();

                    clearTimeout(toolTipTimer);
                    toolTipMouseIsOver = $parents.filter('.' + DOM_CLASS_TOOLTIP).length > 0;
                    if (toolTipMouseIsOver) return;

                    var parallelParents = $target.firstParents(35).andSelf();
                ...
                $(document).mouseout(function(event) {
                    $(event.target).parent().andSelf().filter('.ui-button:not(.ui-state-disabled)').removeClass('ui-state-hover');
                });
